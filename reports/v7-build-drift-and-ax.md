# Codex App Computer Use 全链路逆向 V7
...
```text
20-scenario unified production matrix
64 total steps
186 approval-store post-call checks
...
```text
lab app:
  a7a052ab3141a61008fcb744a948f30e9bf8d9621f35ada31b5db43dcfad4eed
...
5. virtual cursor overlay witness，区分有/无 cursor 的 synthetic CG；
...
9. `ApplicationTarget.identifier(for:)` 的完整 URL canonicalization；
10. Responses 服务端如何允许仅由 `tool_search_output` 暴露的 deferred function；
11. current production request 中显式 Computer Use SKILL body 的逐字节 wire capture；
12. appController 是否存在会替换并清空 `lastAXTree` 的间接分支。
